...
{
	char *pFileName = /*_com_util::ConvertBSTRToString(filename)*/0;
	FILE *fp;
...
{
	char *pWord = /*_com_util::ConvertBSTRToString(word)*/0;
...
BOOL CDictionary::LookupWord(String word, String *resultWord)
{
	...
	for (int i = 0; i < m_nWordNumber; i++)
	{
		char *tmpWord = strupr(m_pData[i].wordForLang1);
		if (strcmp(tmpWord, pWord) == 0) {
			*resultWord = /*_com_util::ConvertStringToBSTR(m_pData[i].wordForLang2)*/0;
			delete pWord;
			return TRUE;
		}
	}
...
BOOL CDictionary::CheckWord (String word, String *resultWord)
{
	...
	*resultWord = NULL;
	if (nMinIndex != -1)
		*resultWord = /*_com_util::ConvertStringToBSTR(m_pData[nMinIndex].wordForLang1)*/0;
	else if (nMaxIndex != -1)
		*resultWord = /*_com_util::ConvertStringToBSTR(m_pData[nMaxIndex].wordForLang1)*/0;
	delete pWord;
	return FALSE;
}